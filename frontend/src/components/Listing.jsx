...
export default function Listing() {
    ...
    useEffect(() => { //fetch api connects to server
        async function fetchData() { //GET (ONE)
            const id = params.id?.toString() || undefined;//? if no id, set undefined
            if (!id) return;
            setIsNew(false);
            const response = await fetch(`http://localhost:3001/listing/${id.toString()}`);
            if (!response.ok) { console.error(`Error: ${response.statusText}`); return; }
            const listing = await response.json();
            if (!listing) {
                console.warn(`Listing with ID = ${id} does not exist, returning to listings`);
                navigate("/");
                return;
            }
            setForm(listing);
    ...
    async function handleSubmit(e) {
        ...
        try {
            let response;
            if (isNew) { //if adding new entity
                response = await fetch("http://localhost:3001/listing", {
                    method: "POST",
                    headers: {
                ...
                response = await fetch(`http://localhost:3001/listing/${params.id}`, {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json",
                    },
        ...
        <>
            ...
            <div className="EntityEdit">
                <form onSubmit={handleSubmit} className="ListingForm">
                    ...
                    <input type="number" name="price" id="price" placeholder="1.00"
                        value={form.price} onChange={(e) => updateForm({ price: e.target.value })}>
                    </input>
                    <label htmlFor="Seller">
                        Seller
                    </label>
                    <input type="text" name="seller_name" id="seller_name" placeholder="First Last"
                        value={form.sellerName} onChange={(e) => updateForm({ sellerName: e.target.value })}>
                    </input>
                    {isNew ?
